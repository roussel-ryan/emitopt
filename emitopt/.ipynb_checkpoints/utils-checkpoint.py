    ...
    return torch.sum(
        (
            post_path_emit_squared(
                post_paths, beam_energy, q_len, distance, x_tuning, meas_dim, X_meas, samplewise=True
            )[0]
        ).abs()
    )
...
def post_path_emit_squared(
    ...
    if samplewise:
        ...
        (
            ...
            emits_squared,
            is_valid,
        ) = compute_emits(k_meas, ys, q_len, distance)[:3]
...
def compute_emit_from_single_beamsize_scan_numpy(k, y, q_len, distance, visualize=False, tkwargs=None):
    ...
    (emit, 
     ...
     is_valid, 
     abc, 
     sigma) = compute_emits(k, y.pow(2).reshape(1,-1), q_len, distance)
...
def get_valid_emit_samples_from_quad_scan(k, y, q_len, distance, 
    ...
    (
        ...
        abc_k_space,
        sigmas_all
    ) = compute_emits(k_virtual, bss, q_len, distance)
...
def plot_parabolic_fits(k, y, abc, ci=0.95, tkwargs=None):
    ...
    if abc.shape[0] > 1:
        ...
        upper_quant = torch.quantile(bss_fit, q=0.5 + ci/2., dim=0)
        lower_quant = torch.quantile(bss_fit, q=0.5 - ci/2., dim=0)
        fit = plt.fill_between(k_fit.detach().numpy(), 
                               lower_quant, 
                               upper_quant, 
                               alpha=0.3, 
                               label='\"Bayesian\" Fit', 
...
def get_valid_emittance_samples(
    ...
    (
        ...
        is_valid,
        abc_k_space
    ) = compute_emits(ks, bss, q_len, distance)[:4]
    sample_validity_rate = (torch.sum(is_valid) / is_valid.shape[0]).reshape(1)
