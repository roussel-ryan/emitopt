...
def build_quad_rmat(k, q_len, thick=True):
    if thick:
        ...
        c = (torch.cos(sqrt_k*q_len)*(k > 0) + torch.cosh(sqrt_k*q_len)*(k < 0) 
            + torch.ones_like(k)*(k == 0)
            )
        s = (1./sqrt_k * torch.sin(sqrt_k*q_len)*(k > 0) + 1./sqrt_k * torch.sinh(sqrt_k*q_len)*(k < 0) 
             + q_len*torch.ones_like(k)*(k == 0)
            )
        cp = (-sqrt_k * torch.sin(sqrt_k*q_len)*(k > 0) + sqrt_k * torch.sinh(sqrt_k*q_len)*(k < 0)
              + torch.zeros_like(k)*(k == 0)
             )
        sp = (torch.cos(sqrt_k*q_len)*(k > 0) + torch.cosh(sqrt_k*q_len)*(k < 0)
              + torch.ones_like(k)*(k == 0)
             )
